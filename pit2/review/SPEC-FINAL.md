# SPEC-FINAL: Figur design fixes (JOINT: R1 + R2 signed)

**Base commit: ad007aa** (origin/claude/leftover-tonight-tasks-5wm6yl).

**Sonnet: build ONLY J-1..J-21.**
- Make one commit per item, with the message `J-n: <title>`.
- Run that item's acceptance test after each commit.
- Build nothing from §3 or §4.
...
**R2-12 / R1-10:** Tony decided on option (b), built as J-20.
...
- With focus on "Add Lamp", press Escape. `.palette.open` count = **0**, and `document.activeElement.getAttribute('aria-label') === 'Open parts'`.
- Open the palette with Enter, press Tab until focus reaches `BODY`, then press Escape. `.palette.open` count = **0**. There is no focus check in this case.
...
**New code:**
```jsx
// above the return: the tab stop is the live row, or the first rendered row when the live row is windowed out
...
- Press ArrowDown twice. The live row is now row 2, which means A=1 and B=0. The aria-labels `Switch A, on` and `Switch B, off` must both exist, and the active element must be the TR with `data-row="2"`.
- With 5 switches (add 3), scroll `.truth .tt` to the bottom so row 0 is not rendered, then Tab into the table. The active element's tagName = `TR`.
...
- In `Palette.jsx`, change the pal-item `onClick` to `onClick={(e) => { if (off) return; onAdd(it); if (e.detail > 0) setOpen(false); }}`. A pointer click-add closes the bar. Keyboard Enter (`e.detail === 0`) keeps it open, so keyboard users can add several parts in a row.
- Extra acceptance steps:
  - Mouse-click "Add AND": `.palette.open` count = 0.
  - Reopen the palette with Enter, press Enter on "Add Switch": `.palette.open` count = 1.

**Acceptance test:**
- Add Switch x4, then Add Lamp, reopening the palette before each click, since a click-add closes it.
...
- With 5 switches at 1440, `tt.getBoundingClientRect().bottom <= 726` (that is 746 − 20u). If it goes past 726, fix it through the J-3 `room` cap. Never change the 54u.
...
**New:**
```css
.node .remove { display: grid; opacity: 0; pointer-events: none; border-radius: 50%; }
.react-flow__node:hover .node .remove, .react-flow__node.selected .node .remove, .node .remove:focus-visible { opacity: 1; pointer-events: all; }
...
- At rest (no hover, no selection), `elementFromPoint` at the tile centre must **not** return `.remove`, so there is no invisible delete.
- While hovering the node, `elementFromPoint(center.x + 20, center.y)` of the tile returns the `.remove` button.
...
- **New:** replace both `var(--u)` in the width/height with `max(var(--u), 1px)`. Do not use `--f`: it equals `--u` at 100% zoom, so it would change nothing.
- **Acceptance test:**
  - At 1280, the Pillow cap-height run in the toast is **≥ 12 px** (it is 11 now). At 1440 and 1920 it is unchanged (12 and 17 ±1).
  - At 1280 the box grows about 12.5%, and its right and bottom edges must still sit 20u (17.8 px ±1) from the canvas rules, measured with Pillow.
...
### Owner-decided items (Tony, after review)

#### J-20. Live-row digits in ink
- **Sources:** D-2 → option (b), R2-12, R1-10.
- **File:** `src/theme.css:306`.
- **Current:** `.truth tr.live td { background: var(--one); color: var(--paper); }`
- **New:** `.truth tr.live td { background: var(--one); color: var(--ink); }`
- **Acceptance test:**
  - Screenshot at 1440 and crop the live row (the first `tr.live`).
  - Pillow: background = the modal colour, which must be (255,90,31). Digit colour = the darkest 5% of pixels, averaged, which must be within 10 of (17,17,17).
  - The WCAG contrast of those two is **≥ 4.5:1**. The expected value is 6.06.
  - Repeat at 1280 and 1920.
- **Must not change:** the `--one` value, the orange row fill, the digit weight and size, the non-live rows, the J-2 focus ring (ink on orange stays visible because of its 3 px inset).

#### J-21. Table overflow cues: frozen header, one centred bottom cue with the sidebar fade, no top cue
- **Source:** Tony. He asked for four things:
  - the header row stays frozen;
  - the vertical cue is at the BOTTOM only, horizontally centred;
  - it gets the same gradient/fade as the palette sidebar;
  - the top arrow goes.
  The left and right cues stay unchanged.
- **Files:** `src/Truth.jsx:65` (the `.tt` div) and `:86` (`<ScrollCues>`); `src/theme.css:158-163` (the palette mask) and `:285` (the truth cue position).

**New code:**
1. Truth.jsx:65. Current: ``className={`tt ${classic ? 'classic' : ''}`}``. New: ``className={`tt ${classic ? 'classic' : ''} ${more.down ? 'fd' : ''}`}``
2. Truth.jsx:86. Current: `<ScrollCues more={more} …/>`. New: `<ScrollCues more={{ ...more, up: false }} …/>`. The top cue never renders "on". Keep the rest of the props exactly.
3. theme.css:158-163. Reuse the sidebar fade *exactly* by widening its selectors. Do not copy the values.
   - `.pal-list { … --fa: 0px; --fb: 0px; -webkit-mask-image: …; mask-image: …; }`: split the mask declarations into a shared rule:
     ```css
     .pal-list, .truth .tt { --fa: 0px; --fb: 0px;
       -webkit-mask-image: linear-gradient(to bottom, transparent calc(var(--fa) * .5), #000 var(--fa), #000 calc(100% - var(--fb)), transparent calc(100% - var(--fb) * .5));
               mask-image: linear-gradient(to bottom, transparent calc(var(--fa) * .5), #000 var(--fa), #000 calc(100% - var(--fb)), transparent calc(100% - var(--fb) * .5)); }
     ```
     Leave the `.pal-list`'s other declarations (height, overflow, padding, box-sizing) in its own rule.
   - Line 163. Current: `.pal-list.fd { --fb: calc(40 * var(--u)); }`. New: `.pal-list.fd, .truth .tt.fd { --fb: calc(40 * var(--u)); }`. Leave `.pal-list.fu` as it is. The table gets no top fade: the header is frozen, so nothing fades under it.
4. theme.css, after line 285, add:
   ```css
   .truth .pal-more.down { left: 0; right: 0; width: auto; top: auto; bottom: 0; }   /* centred bottom cue over the fade, as in the sidebar */
   ```
   The base `.pal-more` already has `height: 20u; place-items: center`, and `.pal-more svg` is 16x8u, so the arrow comes out identical to the sidebar's.
5. The frozen header is already built (`.truth thead th { position: sticky; top: 0 }`). Change nothing there, only verify it.

**Acceptance test:** at 1440, with 5 switches, J-3 and J-6 applied, wheel +100 x2 over `.tt`.
- **Header frozen:** the first `thead th` rect top equals `.tt` rect top ±1. OCR of the header band reads the column letters (for example "A B C D").
- **Bottom cue centred:** the `.truth .pal-more.down.on` count is 1. The centre of its svg rect equals the `.tt` rect centre x **±1 px**. Pillow: the arrow's ink bbox centre x equals the table centre ±1.
- **No top cue:** the `.truth .pal-more.up.on` count is 0, and Pillow finds no ink in the 20u band to the right of the table top (x = tt.right…tt.right+20u, y = tt.top…tt.top+20u).
- **Fade:** Pillow at the table's horizontal centre column. The pixel at `tt.bottom − 5 px` (inside the fade) is lighter than the same row's unmasked digit ink: min grey ≥ 150. The pixel at `tt.bottom − 60 px` still has digit/rule ink with min grey < 60.
- **Fade off at the end:** scroll to the bottom. `.tt.fd` count = 0 and the `.down` cue has no `on` class.
- **Left/right cues:** unchanged. Their rects are identical to the pre-change run ±1.
- **Sidebar regression:** the palette open at 1440 gives a Pillow crop of the bar bottom identical to `r1/1440-02-palette-hover.png` (ink diff < 1%).

**Must not change:** the palette's cue and fade look, the left/right table cues, the sticky header, the row snap, the J-3 row sizing.

---

## 3. Open decisions for Tony (not built)

D-2 was decided (built as J-20). D-1 and D-4 moved to §4.
...
- **D-1, grid disk (R2-9):** not built. Tony is keeping it for the dating mode.
- **D-4, row 03 (R1-12 / R2-10):** DO NOT TOUCH. Tony has reserved it for the pending undo/redo bar.
