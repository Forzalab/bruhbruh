...
Variant v1:
- Colour only: lamp columns (th + td) on `--grid` (existing field grey); switch columns stay paper. Links to the canvas: the switch is the paper square you press, the lamp is where the result shows, so the result columns get the page's one field tone. `--one` stays logic-1 only (live row still overrides). New tokens: 0.
