...
//
// Run: node scripts/e2e.mjs   (starts its own vite dev server on :5391)
...
async function testThreeInput(page) {
  ...
  const clicked = await page.evaluate((lbl) => {
    const row = [...document.querySelectorAll('aside.truth tbody tr:not(.pad)')]
      .find((tr) => tr.querySelector('td:first-child .sr')?.textContent === lbl);
    if (!row) return false;
    row.click();
    return true;
  }, label);
...
async function testThirteenSwitches(page) {
  ...
  const firstLabel = await page.evaluate(() => document.querySelector('aside.truth tbody tr:not(.pad) td:first-child .sr')?.textContent);
  ...
  await page.waitForTimeout(50);
  await page.waitForTimeout(50);
  const lastLabels = await page.evaluate(() => [...document.querySelectorAll('aside.truth tbody tr:not(.pad) td:first-child .sr')].map((s) => s.textContent));
  ...
  for (const rowIndex of [0, 1, 4096, 8191]) {
    ...
    await page.waitForTimeout(50);
    const clicked = await page.evaluate((lbl) => {
      const row = [...document.querySelectorAll('aside.truth tbody tr:not(.pad)')].find((tr) => tr.querySelector('td:first-child .sr')?.textContent === lbl);
      if (!row) return false;
      row.click();
      return true;
    }, label);
