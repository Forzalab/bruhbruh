...
// the number of PATHS, not nodes. These are real budgets a truth table needs; they fail today. Marked todo so
// `npm test` stays green; drop the todo flag once evaluate() is topological/memoised.
test('BUG-1a: 13-input balanced AND tree, full 8192-row table under 1 s', { todo: 'BUG-1 evaluate() is path-exponential' }, () => {
  const b = builder(); const ins = Array.from({ length: 13 }, () => b.sw());
...
test('BUG-1b: 16-gate reconvergent ladder (g_i = AND(g_{i-1}, g_{i-1})) evaluates once in under 50 ms', { todo: 'BUG-1 evaluate() is path-exponential' }, () => {
...
test('BUG-2: canConnect accepts non-integer / string pins', { todo: 'BUG-2 pin is not validated as an integer' }, () => {
